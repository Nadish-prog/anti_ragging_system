...
const mapComplaint = (c) => {
  ...
  return {
    ...
    evidence: c.evidence || [],
    timeline: c.complaint_logs?.map(log => ({
      id: log.log_id,
      action: log.action_description,
      timestamp: log.timestamp,
      userId: log.action_by_user_id
    })) || []
  }
}
    ...
    try {
        ...
        if (err.response?.status === 403 || err.response?.status === 404) {
            try {
                ...
                const response = await api.get(`/complaints/my-complaints/${id}`)
                return { complaint: mapComplaint(response.data.complaint) }
            } catch (studentErr) {
                // Try fetching from lists (faculty fallback)
                const assigned = await reportService.getAssignedReports().catch(() => ({complaints: []}))
                const found = assigned.complaints.find(c => c.id.toString() === id.toString())
